...
Future<List<Product>> fetchProducts() async {
  final response = await http.get(Uri.parse('http://localhost:8888/restful_api_php/api/sp/quan.php'));
...
class CategoryQuanPage extends StatefulWidget {
  final User user;
  final Cart cart;
  CategoryQuanPage({super.key, required this.user, required this.cart});


  @override
  State<CategoryQuanPage> createState() => _ListProductsPageState();
}
class _ListProductsPageState extends State<CategoryQuanPage> {
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => CartPage(user: widget.user,cart: ,)),
              );
            },
          ),
          const SizedBox(
